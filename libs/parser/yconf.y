...
shaveapp_entry_start: T_SHAVEAPP T_WORD T_EOL
{
  ...
  char shaveapp_settings_file[PATH_MAX+1];
  snprintf(shaveapp_settings_file,
    sizeof(shaveapp_settings_file)/sizeof(shaveapp_settings_file[0]),
    "shaveapp-%s.kcnf", $2);
  zconf_nextfile(shaveapp_settings_file); // this is generated by the build system
...
void shaveapp_generate_placement(const char *shaveapp_id)
{
  ...
  const int SHAVE_CORE_COUNT=16; // TODO lookup the specially defined symbol and get it's value here
  int i;
